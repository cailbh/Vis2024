...
export default {
  ...
  mounted () {
    ...
    if (this.showPlay) {
     ...
     console.log(this.$refs.veo,this.$refs.veo.clientWidth,this.$refs.veo.clientHeight)
    }
    if (this.autoplay) {
      this.vplay = true
  ...
  methods: {
    ...
    getVidDir(){
      // console.log("ready",this.$refs.veo.duration)
      this.$bus.$emit("VideoDur", this.$refs.veo.duration);
    },
    onPause () {
      this.vplay = false
    ...
    timeupdate(){
      ...
      this.$emit("videoTime", parseFloat(time));
    }
  }
}
</script>
